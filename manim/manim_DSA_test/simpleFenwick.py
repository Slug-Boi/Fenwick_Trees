...
class Test(Scene):
    def construct(self):
        ...
        for num in array:
            ...
            self.play(Create(box), Create(numText), run_time=0.5)
        ...
        self.play(
            ...
            Transform(boxes[1][1], Text("12", stroke_width=3).move_to(newLevel.get_center())),
            rate_func=linear,
            run_time=0.6
        )

        newNewLevel = (
            Rectangle(height=1, width=2.27*2) # 2.27 is the width of 2 square with the gap
            .to_edge(LEFT)
            .to_edge(DOWN)
            .shift(UP*UPLEVEL*2)
        )
        ...
        self.play(
            ...
            Transform(boxes[3][1], Text("19", stroke_width=3).move_to(newNewLevel.get_center())),
            rate_func=linear,
            run_time=0.75
        )
